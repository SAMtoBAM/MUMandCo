...
cat $prefix.gaps_1000filtered ""$prefix"_ref".coords_dups_1000 ""$prefix"_query".coords_dups_1000 $prefix.transloc_candidate_alignments6_fragments10000 $prefix.1000bp_inversionsclean | sort -k1,1 -k3V  > $prefix.1000bp
...
cat $prefix.gaps_1000filteredinv ""$prefix"_ref".coords_dups_1000 ""$prefix"_query".coords_dups_1000 $prefix.transloc_candidate_alignments6_fragments10000 $prefix.1000bp_inversionsclean  | sort -k1,1 -k3V  > $prefix.1000bp2
...
cat header.txt ${prefix}.SVs_all.withfragmentTEMP2.tsv | awk '{if($9 == "complicated" || $9 == "double"){t=$9; $9=$10; $10=t; print} else {print $0}}' | sed 's/ /\t/g' > ${prefix}.SVs_all.withfragment.tsv
...
echo "##ALT=<ID=TRA,Description="Region involved in translocation">" >> ${prefix}.SVs_all.vcf
...
do
	##split up the line into individual features to fit it into the VCF format
		echo $variant | awk '{if($6 == "deletion") {print $1"\t"$3"\t.\t"$9"\t<DEL>\t.\tPASS\tEND="$4";SVLEN=-"$5";SVTYPE=DEL;qCHR="$2";qSTART="$7";qEND="$8"\tGT\t1/1"}\
								...
								if($6 == "transloc") {print $1"\t"$3"\t.\t"$9"\t<TRA>\t.\tPASS\tEND="$4";SVLEN="$5";SVTYPE=TRA;qCHR="$2";qSTART="$7";qEND="$8"\tGT\t1/1"}}' >> ${prefix}.SVs_all.vcf
done
