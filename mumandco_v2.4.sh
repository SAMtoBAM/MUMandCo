...
if [[ "$average_quality" == "100" ]]
	then
		...
		awk 'NR==FNR{a[$1,$2]++;next};a[$14,$15] {print $0}' ""$prefix"_chromosome_pairs.txt" "$alignments_folder/""$prefix"_ref".delta_filter.coordsg" > ""$prefix"_ref".coordsg_matched
		awk 'NR==FNR{a[$1,$2]++;next};a[$15,$14] {print $0}' ""$prefix"_chromosome_pairs.txt" "$alignments_folder/""$prefix"_query".delta_filter.coordsg" > ""$prefix"_query".coordsg_matched
...
cat $prefix.transloc_candidate_alignments2 | awk 'BEGIN{chrr=""; chrq=""; startr=0; endr=0; startq=0; endq=0; link=0}\
												...
												else if(chrr!=$14){print $0"\t"chrr"\t"chrq"\t"startr"\t"endr"\t"endr-startr"\ttransloc\t"startq"\t"endq; startr=$1; endr=$2; startq=$3; endq=$4; chrr=$14; chrq=$15}}\
...
cat ""$prefix"_ref".gaps_1000 ""$prefix"_query".gaps_1000 ""$prefix"_ref".inv_1000_neighbours_agg $prefix.transloc_candidate_alignments6_fragments10000 | sort -k1,1 -k3V  > $prefix.1000bp_invfilt
...
cat $prefix.gaps_1000filteredinv ""$prefix"_ref".coords_dups_1000 $prefix.transloc_candidate_alignments6_fragments10000 $prefix.1000bp_inversionsclean  | sort -k1,1 -k3V  > $prefix.1000bp2
