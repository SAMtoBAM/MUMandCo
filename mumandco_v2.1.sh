...
then
	$NUCMER --maxmatch --nosimplify -p ""$prefix"_ref" $reference_assembly $query_assembly
	...
	$NUCMER --maxmatch --nosimplify -p ""$prefix"_query" $query_assembly $reference_assembly
	...
	then
		$NUCMER --maxmatch -l 100 -c 500 -p ""$prefix"_ref" $reference_assembly $query_assembly
		$DELTAFILTER -m ""$prefix"_ref".delta > ""$prefix"_ref".delta_filter
		$SHOWCOORDS -T -r -c -l -d -g ""$prefix"_ref".delta_filter > ""$prefix"_ref".delta_filter.coordsg
		$SHOWCOORDS -T -r -c -l -d ""$prefix"_ref".delta_filter > ""$prefix"_ref".delta_filter.coords
	
		$NUCMER --maxmatch -l 100 -c 500 -p ""$prefix"_query" $query_assembly $reference_assembly
		$DELTAFILTER -m ""$prefix"_query".delta > ""$prefix"_query".delta_filter
		$SHOWCOORDS -T -r -c -l -d -g ""$prefix"_query".delta_filter > ""$prefix"_query".delta_filter.coordsg
		$SHOWCOORDS -T -r -c -l -d ""$prefix"_query".delta_filter > ""$prefix"_query".delta_filter.coords
	else
		echo "My what a large genome you have, this may take some time"
		$NUCMER --maxmatch -l 500 -c 500 -p ""$prefix"_ref" $reference_assembly $query_assembly
		$DELTAFILTER -m ""$prefix"_ref".delta > ""$prefix"_ref".delta_filter
		$SHOWCOORDS -T -r -c -l -d -g ""$prefix"_ref".delta_filter > ""$prefix"_ref".delta_filter.coordsg
		$SHOWCOORDS -T -r -c -l -d ""$prefix"_ref".delta_filter > ""$prefix"_ref".delta_filter.coords
	
		$NUCMER --maxmatch -l 500 -c 500 -p ""$prefix"_query" $query_assembly $reference_assembly
		$DELTAFILTER -m ""$prefix"_query".delta > ""$prefix"_query".delta_filter
		$SHOWCOORDS -T -r -c -l -d -g ""$prefix"_query".delta_filter > ""$prefix"_query".delta_filter.coordsg
		$SHOWCOORDS -T -r -c -l -d ""$prefix"_query".delta_filter > ""$prefix"_query".delta_filter.coords
	fi
...
cat ""$prefix"_ref".coordsg_matched_subtelofilt | awk 'BEGIN{chr=""; start=""; stop=""}; \
										...
										> ""$prefix"_ref".coordsg_matched_subtelofilt2

cat ""$prefix"_ref".coordsg_matched_subtelofilt2 |\
awk 'BEGIN{chrr=""; chrq=""; startr=0; endr=0; startq=0; endq=0; link=0} {if(chrr=="" && chrq=="") {chrr=$14; chrq=$15; startr=$1; endr=$2; startq=$3; endq=$4} \
												...
												>""$prefix"_ref".fragments

#remove many small fragments and join the large fragments which they broke"
cat ""$prefix"_ref".fragments | awk '!/contig/{if($5< 500) print $1"\t"$2"\t"$3"\t"$4"\t"$5"\ttransloc_500"$6"\t"$7}' > ""$prefix"_ref".fragments_less500
cat ""$prefix"_ref".fragments | awk '!/contig/{if($5>=500) print $0}'> ""$prefix"_ref".fragments_more500

##AGAIN remove smaller false translocations that lie inside the larger true fragment then concatenate again
cat ""$prefix"_ref".fragments_more500 | awk 'BEGIN{chr=""; start=""; stop=""}; \
...
cat $prefix.gaps_50filtered ""$prefix"_ref".coords_dups_50 | sort -V -k1 -k3 | sort -V -k1  |\
...
cat $prefix.gaps_1000filtered ""$prefix"_ref".coords_dups_1000 ""$prefix"_ref".fragments_more500concat $prefix.1000bp_inversionsclean | sort -k1,1 -k3V  > $prefix.1000bp
...
cat $prefix.gaps_1000filteredinv ""$prefix"_ref".coords_dups_1000 ""$prefix"_ref".fragments_more500concat $prefix.1000bp_inversionsclean ""$prefix"_ref".coordsg_matched_largeinv  | sort -k1,1 -k3V  > $prefix.1000bp2
